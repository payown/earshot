...
/// Manual V1→V2 store migration (issue #355).
///
/// SwiftData's lightweight migration can't add the non-optional attributes the
/// V2 model introduces, so instead of a `SchemaMigrationPlan` we read the old
/// store through ``EarshotSchemaV1``, snapshot it into plain values, replace the
/// store file, and reinsert the data as V2 objects. This preserves the tester's
/// subscriptions, episodes, and played state while keeping the V2 model clean.
enum StoreMigration {
    ...
    /// Opens the store at `url` as V2, performing a manual V1→V2 migration first
    /// if the on-disk store is still at the original schema. Throws if the store
    /// can be opened as neither V2 nor V1.
    @MainActor
    static func openOrMigrate(at url: URL) throws -> ModelContainer {
        let schema = Schema(versionedSchema: EarshotSchemaV2.self)

        // Fresh installs and already-migrated stores open directly.
        ...
        // Otherwise treat it as an original (V1) store and migrate it.
        ...
        AppLog.data.info("Migrated \(snapshots.count) podcast(s) from V1 to V2")
        return container
    }
