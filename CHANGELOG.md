...
### Fixed
- Large libraries: fixed the phone running hot during long listening sessions. After the earlier crash fix, the Inbox tab's unread badge still re-counted your unplayed episodes every few seconds while you listened (once per position save, and more often at faster speeds), and on a big backlog each recount did real work. Over an hour or two that added up to enough sustained processor use to warm the phone. The badge now does no work at all as you listen — it recounts only when the inbox actually changes (a new episode arrives, or you play, queue, dismiss, or include/exclude a show) and refreshes when you return to the app. The count it shows is unchanged. (#736)
