...
final class EpisodeFilterTests: XCTestCase {
    ...
    func testKeepMatchingWithNoEnabledRulesBypassesIngest() {
    ...
    func testDurationSaveAssessmentRefusesNoDurationsAndWarnsOnPartialCoverage() {
        let configuration = EpisodeFilterConfiguration(
            isEnabled: true,
            mode: .keepMatching,
    ...
    func testApprovedVoiceOverRuleAndPreviewReadingOrder() {
        let rule = EpisodeFilterRule(
            name: "Full shows",
            isEnabled: true,
            titlePattern: "*show*",
